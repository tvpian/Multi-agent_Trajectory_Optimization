...
#include <coupled_optimization.hpp>
...
int main(int argc, char *argv[])
{
    ...
    std::shared_ptr<CoupledMPCPlanner> MPCPlanner_obj;
    ...
    while (ros::ok())
    {
        if (has_octomap && !has_path)
        {
            ...
            timer_total.reset();
            ROS_INFO("Multi-robot Trajectory Planner");
            ...
            timer_step.reset();
            {
                corridor_obj.reset(new SafeCorridor(initTrajPlanner_obj, distmap_obj, mission, param));
            ...
            timer_step.reset();
            {
                MPCPlanner_obj.reset(new CoupledMPCPlanner(corridor_obj, initTrajPlanner_obj, mission, param));
            ...
            resultPublisher_obj.reset(new ResultPublisher(nh, corridor_obj, initTrajPlanner_obj, mission, param));
