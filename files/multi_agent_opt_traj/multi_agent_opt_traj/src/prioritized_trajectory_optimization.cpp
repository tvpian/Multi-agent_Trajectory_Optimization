...
#include <prioritized_optimization.hpp>
...
int main(int argc, char *argv[])
{
    ...
    std::shared_ptr<PrioritizedMPCPlanner> MPCPlanner_obj;
    ...
    while (ros::ok())
    {
        if (has_octomap && !has_path)
        {
            ...
            timer_step.reset();
            {
                MPCPlanner_obj.reset(new PrioritizedMPCPlanner(corridor_obj, initTrajPlanner_obj, mission, param));
