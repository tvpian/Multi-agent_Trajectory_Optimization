...
public:
  ...
  void operator()(ADvector &fg, const ADvector &vars)
  {
    ...
    for (int i = 0; i < relative_pair.size(); i++)
    {
      fg[cont] = CppAD::pow(vars[x_start + N * relative_pair[i].at(1) + relative_pair[i].at(0)] \
                              - vars[x_start + N * relative_pair[i].at(2) + relative_pair[i].at(0)], 2) \
                              + CppAD::pow(vars[y_start + N * relative_pair[i].at(1) + relative_pair[i].at(0)] \
                              - vars[y_start + N * relative_pair[i].at(2) + relative_pair[i].at(0)], 2);
      cont++;
    }
  }
};

class CoupledMPCPlanner : public MPCPlanner
{
public:
  ...
  CoupledMPCPlanner(std::shared_ptr<SafeCorridor> _corridor_obj,
             std::shared_ptr<InitTrajPlanner> _initTrajPlanner_obj,
             TrajPlanning::Mission _mission,
             TrajPlanning::Param _param)
      : MPCPlanner(_corridor_obj, _initTrajPlanner_obj, _mission, _param)
  {
    int i, j, k;
  ...
  void Solve()
  {
    ...
    // solve the problem
    CppAD::ipopt::solve<Dvector, VectorConstraint>(
