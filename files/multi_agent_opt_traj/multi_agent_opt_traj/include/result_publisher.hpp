...
public:
    ResultPublisher(ros::NodeHandle _nh,
                    std::shared_ptr<SafeCorridor> _corridor_obj,
    ...
    {
        update_init_trajectory();
        update_obstacle_box(current_time);
        update_trajectory(current_time);
...
private:
    ...
    void update_trajectory(double current_time)
    {
    ...
    void update_obstacle_box(double current_time)
    {
        visualization_msgs::MarkerArray mk_array;
        for (int qi = 0; qi < qn; qi++)
        {
        ...
        for (int qi = 0; qi < qn; qi++)
        {
            ...
            for (int i = 0; i < 2; i++)
            {
                ...
                if (i == 0)
                {
                    ...
                    mk.mesh_resource = std::string("package://multi_agent_opt_traj/chassis.dae");
